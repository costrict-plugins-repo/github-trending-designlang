...
export function formatFramerMotion(design) {
  ...
  // Motion v3: prefer a real, runtime-observed stagger step over the heuristic.
  ...
  const staggerSec = (observedStagger != null ? observedStagger : Math.max(0.04, defaultDur * 0.25)).toFixed(3);
  ...
  lines.push('  stagger: {');
  lines.push('    hidden: {},');
  lines.push(`    show:   { transition: { staggerChildren: ${staggerSec} } },${observedStagger != null ? ' // observed at runtime' : ''}`);
  lines.push('  },');
